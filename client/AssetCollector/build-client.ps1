...
$updateExeName = 'it-asset-client-' + ([System.Diagnostics.FileVersionInfo]::GetVersionInfo($exePath).FileVersion) + '.exe'
...
$manifest = [ordered]@{
  ...
  notes = 'Auto-published by build-client.ps1'
}
if ($env:ASSET_UPDATE_SIGNING_PRIVATE_KEY) {
  ...
  if ($LASTEXITCODE -ne 0 -or -not $signature) {
    throw 'Failed to sign client update manifest.'
  }
  $manifest.signature = $signature.Trim()
}
$manifestJson = $manifest | ConvertTo-Json -Depth 4
[System.IO.File]::WriteAllText((Join-Path $dataDir 'client-update.json'), $manifestJson, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "Built $exePath"
Write-Host "Published update manifest to $(Join-Path $dataDir 'client-update.json')"

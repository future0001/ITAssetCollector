...
namespace AssetCollector
{
    public static class HardwareCollector
    {
        ...
        private static List<NetworkAdapterInfo> GetPhysicalNetworkAdapters()
        {
            ...
            if (list.Count == 0 && IsWindowsXpOrOlder())
            {
                foreach (int index in configs.ByIndex.Keys)
                {
                    List<string> ips = configs.ByIndex[index];
                    if (ips == null || ips.Count == 0) continue;

                    var item = new NetworkAdapterInfo();
                    item.name = configs.ByIndexName.ContainsKey(index) ? configs.ByIndexName[index] : "Network Adapter";
                    item.macAddress = configs.ByIndexMac.ContainsKey(index) ? configs.ByIndexMac[index] : string.Empty;
                    AddUniqueIps(item.ipAddresses, ips);
                    if (string.IsNullOrEmpty(item.macAddress) && item.ipAddresses.Count == 0) continue;
                    list.Add(item);
                }
            }
        ...
        private static NetworkConfigMaps GetNetworkConfigMaps()
        {
            ...
            foreach (ManagementObject obj in Query("SELECT Index, MACAddress, IPAddress, IPEnabled FROM Win32_NetworkAdapterConfiguration"))
            {
                ...
                if (index >= 0 && !string.IsNullOrEmpty(mac)) result.ByIndexMac[index] = mac;
                if (index >= 0 && !string.IsNullOrEmpty(description)) result.ByIndexName[index] = description;
                if (!string.IsNullOrEmpty(mac)) result.ByMac[mac] = ips;
                if (!string.IsNullOrEmpty(mac) && !string.IsNullOrEmpty(description)) result.ByMacName[mac] = description;
            }
            return result;
        }
        ...
        private static List<DiskInfo> GetPhysicalDisks()
        {
            ...
            foreach (ManagementObject obj in QueryDiskDrives())
            {
                int index = ReadInt(obj, "Index");
        ...
        private static List<ManagementObject> QueryDiskDrives()
        {
            if (IsWindowsXpOrOlder())
            {
                List<ManagementObject> xpDisks = Query("SELECT Index, DeviceID, PNPDeviceID, Model, Size, InterfaceType FROM Win32_DiskDrive");
                if (xpDisks.Count > 0) return xpDisks;
                return Query("SELECT * FROM Win32_DiskDrive");
            }

            return Query("SELECT Index, DeviceID, PNPDeviceID, Model, SerialNumber, Size, MediaType, InterfaceType FROM Win32_DiskDrive");
        }
        ...
        private static List<StorageDiskCandidate> GetPnpDiskCandidates()
        {
            ...
            try
            {
                string wql = IsWindowsXpOrOlder()
                    ? "SELECT Name, DeviceID, HardwareID, CompatibleID FROM Win32_PnPEntity WHERE DeviceID IS NOT NULL"
                    : "SELECT Name, DeviceID, PNPClass, HardwareID, CompatibleID FROM Win32_PnPEntity WHERE DeviceID IS NOT NULL";
                foreach (ManagementObject obj in Query(wql))
                {
                    string name = ReadString(obj, "Name");
        ...
        private class NetworkConfigMaps
        {
            ...
            public Dictionary<string, string> ByMacName = new Dictionary<string, string>();
            public Dictionary<int, List<string>> ByIndex = new Dictionary<int, List<string>>();
            public Dictionary<int, string> ByIndexMac = new Dictionary<int, string>();
            public Dictionary<int, string> ByIndexName = new Dictionary<int, string>();
        }

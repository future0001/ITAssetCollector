...
这是一个用于内网 IT 资产采集、台账管理、组织架构展示、IP 资源检查和客户端安装状态管理的小型系统。

系统由两部分组成：

- 服务端：Node.js 单文件服务，提供 Web 管理界面和数据接口。
- 客户端：Windows WinForms 采集程序，支持普通版和 XP 兼容版。

## 主要功能

- 资产台账：查看、搜索、筛选、编辑、删除、批量分类、导出 Excel。
- 可视化概览：按资产分类、部门、缺失字段、异常硬盘进行统计，并可点击下钻到相关资产。
- 缺失字段筛选：快速定位主机名、资产编号、使用人、部门、位置、IP、MAC、主板、硬盘序列号、系统信息等缺失项。
- IP 详情：展示网段使用情况、重复 IP、无 IP 设备，并支持可达性检测。
- 组织图示：展示公司、事业部、部门、小组和员工，支持编辑组织节点和直属员工。
- 客户端管理：查看客户端安装、在线、未检测、未建档心跳、版本发布和采集差异。
- 离线采集：客户端可在网络不可用时保存本地离线记录，之后批量提交。
- 自动发现服务端：客户端支持自动检测内网服务端地址。
...
scripts/seed-demo-data.js        生成演示组织架构、资产和客户端状态
dist/                           打包分发目录
```
...
生产或内网使用时，把客户端里的服务端地址改为实际服务器 IP，例如：
...
安装后客户端会启动后台服务和托盘程序。关闭窗口不会退出后台运行，可通过托盘或桌面快捷方式重新打开客户端界面。
...
少数 Windows 7 或特殊 NVMe 驱动可能只暴露 EUI/NGUID，而不是厂商序列号。可以在客户端配置文件中增加修正规则：
...
- 定期备份 `server/data`。
- 发布客户端前先运行对应 `build-client.ps1`。
- 修改 Web UI 后检查首页、IP 详情、组织图示、客户端管理四个页面。
- 演示前可运行 `node scripts\seed-demo-data.js` 重置演示数据。
- 如果 PowerShell 提示 `oh-my-posh` 不存在，那是本机 PowerShell profile 的提示，不影响本项目构建。
